...
const deploymentMarker = 'hostinger-auto-rebuild-2026-05-18-02';
...
async function bootstrapNext() {
  ...
  try {
    ...
    if (!dev && autoBuildEnabled && (buildNeeded || installNeeded)) {
      buildAttempted = true;
      await runNpm(['run', 'build'], 'building-next-app');
    }

    const prepared = await prepareNext();
    if (!prepared && !dev && autoBuildEnabled && !buildAttempted) {
      buildAttempted = true;
      await runNpm(['run', 'build'], 'rebuilding-after-prepare-failure');
      await prepareNext();
    }
  } catch (error) {
    bootstrapError = error;
...
function runtimePayload(extra = {}) {
  return {
    ...
    guidance: [
      ...
      'If buildExists is false or buildNeeded is true, the bootstrap will run npm run build unless CIH_AUTO_BUILD=0.',
